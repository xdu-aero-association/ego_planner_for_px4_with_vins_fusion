	...
	switch (state)
	{
		case MANUAL_CTRL:
		{
			if ( rc_data.enter_api_mode ) 
			{
				...
				if ( odom_is_received(now_time) )
				{
					...
					ROS_INFO("\033[32m[px4ctrl] Start AUTO_HOVER by px4ctrl!\033[32m");
				}
				else
				{
					ROS_ERROR("[px4ctrl] Reject AUTO_HOVER. No odom!");
				}
			}
		...
		case AUTO_HOVER:
		{
			if ( !rc_data.is_api_mode || !odom_is_received(now_time) )
			{
				...
				ROS_WARN("[px4ctrl] Return to manual control!");
			}
			else if ( rc_data.is_command_mode && cmd_is_received(now_time) )
			{
				if ( state_data.current_state.mode == "OFFBOARD" )
				{
					...
					ROS_INFO("\033[32m[px4ctrl] position commands received!\033[32m");
				}
			}
			else
			{
		...
		case CMD_CTRL:
		{
			if ( !rc_data.is_api_mode || !odom_is_received(now_time) )
			{
				...
				ROS_WARN("[px4ctrl] Return to MANUAL_CTRL!");
			}
			else if ( !cmd_is_received(now_time) )
			{
            	...
            	set_hov_with_odom();
				process_hover_control(u, u_so3);
				ROS_WARN("[px4ctrl] Return to AUTO_HOVER!");
			}
			else
			{
				process_cmd_control(u, u_so3);
...
// void PX4CtrlFSM::publish_desire(const Desired_State_t& des)
// {
// 	geometry_msgs::PoseStamped msg;
